...
def index():
    ...
    for transcript_id in transcript_files:
        ...
        text_preview = read_conversation_text(transcript_id)
        preview = (
            text_preview[:200] + "..." if len(text_preview) > 200 else text_preview
        )
        transcripts_info.append(
            {"id": transcript_id, "has_analysis": has_analysis, "text": preview}
        )
